// Added generateAllReports function for cron jobs
export const generateAllReports = async () => {
  console.log('Generate all reports cron job started');
  try {
    const db = getDb();
    
    // Get all active surveys that need reports
    const surveys = await db.collection('surveys').find({ 
      status: 'active',
      // Add any other criteria for surveys that should have reports generated
    }).toArray();

    console.log(`Found ${surveys.length} surveys to generate reports for`);
    ...
    for (const survey of surveys) {
      try {
        ...
        const existingReport = await db.collection('reports').findOne({
          surveyId: survey._id,
          // You might want to add date criteria here, e.g., created today
        });

        if (existingReport) {
          console.log(`Report already exists for survey ${survey._id}, skipping`);
          continue;
        }
        ...
        // Only generate report if there are responses
        if (responses.length === 0) {
          console.log(`No responses found for survey ${survey._id}, skipping`);
          continue;
        }
        ...
        const newReport = {
          surveyId: survey._id,
          title: `Auto-generated Report: ${survey.title}`,
          ...
          description: `Automatically generated report for survey: ${survey.title}`,
        ...
        if (result.insertedId) {
          console.log(`Successfully generated report ${result.insertedId} for survey ${survey._id}`);
          successCount++;
        } else {
          console.error(`Failed to insert report for survey ${survey._id}`);
          errorCount++;
        }

      } catch (surveyError) {
        console.error(`Error processing survey ${survey._id}:`, surveyError);
        errorCount++;
      }
    }

    console.log(`Generate all reports completed. Success: ${successCount}, Errors: ${errorCount}`);
    
    return {
      success: true,
      message: 'Reports generation completed',
      stats: {
