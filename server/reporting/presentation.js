...
export default class Presentation {
  ...
  generate() {
    ...
    this.reportData.sections.forEach(section => {
      ...
      if (section.slideContent) {
        section.slideContent.forEach(item => {
          slide.addText(item.text, item.options);
        });
      }

      if (section.chartData) {
        slide.addChart(pptx.ChartType.bar, section.chartData, { x: 1, y: 2, w: 8, h: 4 });
      }

      if (!section.slideContent && !section.chartData) {
